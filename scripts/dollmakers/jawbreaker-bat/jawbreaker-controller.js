import { updateCanvas } from '/scripts/dollmakers/jawbreaker-bat/jawbreaker-canvas-controller.js';
import { save_pic } from '/scripts/dollmakers/jawbreaker-bat/jawbreaker-canvas-controller.js';
import { save_lines } from '/scripts/dollmakers/jawbreaker-bat/jawbreaker-canvas-controller.js';
...
function randomButtonsSetup() {
    ...
    const current_trait = document.getElementById("randomize-current-trait");
    ...
    current_trait.addEventListener("click", () => {
        randomize("current-trait");
    })
}
...
function verify() {
    ...
    if (!Object.hasOwn(masterlist["topcoat"], current.topcoat.selected)) {
        current.topcoat.selected = "none";
    }
    if (!Object.hasOwn(masterlist["teeth"], current.teeth.selected)) {
        current.teeth.selected = "teeth";
    }
    if (!Object.hasOwn(masterlist["mouth"], current.mouth.selected)) {
        current.mouth.selected = "mouth";
    }
...
async function loadMasterlist() {
    try {
        const res = await fetch('/scripts/dollmakers/jawbreaker-bat/traits.json');
